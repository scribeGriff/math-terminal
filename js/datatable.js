    ...
    if (fetchData) {
      ...
      if (this.options.data.size !== undefined) {
        ...
        if (this.options.data.allInOne) {
          ...
          for (let i = 0; i < this.options.data.size;
...
DataTable.prototype = {
  ...
  createSelectFilter: function (field) {
    ...
    if (empty) {
      ...
      optioni.dataset.empty = true;
      optioni.value = "";
      optioni.innerHTML = emptyValue;
  ...
  destroySort: function () {
    document.getElementsByTagName('thead th').removeClass('sorting sorting-asc sorting-desc')
  ...
  addRow: function (data) {
    ...
    this.currentStart = parseInt(this._index(this._index(data, this.data),
                                             this.filterIndex) / this.options.pageSize, 10) * this.options.pageSize;
    this.refresh();
  },
